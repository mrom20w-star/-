...
<body class="min-h-screen flex flex-col antialiased select-none custom-scrollbar">
  ...
  <main class="max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 flex flex-col gap-6 flex-1">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
      ...
      <!-- Stat 5: Active Devices -->
      <div class="bg-[#111c33] border border-slate-800/90 rounded-2xl p-4 flex flex-col gap-1 shadow-md hover:border-slate-700 transition">
        <span class="text-xs text-slate-400 font-medium">📱 أجهزة متصلة ومسجلة</span>
        <span class="text-xl sm:text-2xl font-black text-cyan-400 font-mono"><?= $totalUsersCount ?></span>
        <span class="text-[10px] text-slate-500">تحت المراقبة الحية</span>
      </div>
      ...
      <div class="flex items-center justify-between flex-wrap gap-3">
        ...
        <div class="flex items-center gap-2">
          <input 
            ...
            class="bg-[#111c33] border border-slate-700 rounded-xl px-3.5 py-2 text-xs text-white focus:outline-none focus:border-blue-500 w-64 sm:w-80 shadow-inner"
          >
        </div>
      </div>
          ...
          <table class="w-full text-right text-xs" id="devices-table">
            <thead class="bg-[#0b1222] text-slate-400 font-bold border-b border-slate-800 select-none">
              <tr>
                ...
                <th class="p-3.5 text-center">الإجراء (حذف وحظر)</th>
              </tr>
            </thead>
            <tbody id="devices-table-body" class="divide-y divide-slate-800/60 font-medium">
              <?php if (empty($usersList)): ?>
                <tr>
                  <td colspan="8" class="p-8 text-center text-slate-500">لا يوجد مستخدمون مسجلون حالياً في قاعدة البيانات</td>
                </tr>
              <?php else: ?>
                ...
                ?>
                <tr class="hover:bg-slate-800/40 transition device-row <?= $isUserBanned ? 'opacity-40 bg-red-950/10' : '' ?>" data-search="<?= strtolower("{$uname} {$uid} {$uphone} {$devName} {$devId}") ?>">
                  <!-- User Info -->
                  <td class="p-3.5">
                    <div class="flex items-center gap-2">
                  ...
                  <td class="p-3.5 text-center">
                    <?php if ($isUserBanned): ?>
                      <span class="text-[10px] bg-red-500/20 text-red-400 font-bold px-2.5 py-1 rounded-lg border border-red-500/30">
                        محظور حالياً
                      </span>
                    <?php else: ?>
                      <button 
                        ...
                        class="px-3 py-1.5 rounded-xl bg-red-600/90 hover:bg-red-500 text-white font-bold text-xs shadow-md shadow-red-950 transition cursor-pointer flex items-center gap-1 mx-auto hover:scale-105 active:scale-95"
                      >
                        <span>🗑️</span>
                        <span>حذف وحظر نهائياً</span>
                      </button>
                    <?php endif; ?>
                  </td>
                </tr>
                  ...
                  <td class="p-3.5 text-center">
                    <?php if ($txStatus === 'pending'): ?>
                      <div class="flex items-center justify-center gap-1.5">
                        <button onclick="approveDeposit('<?= $txId ?>')" class="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-md transition cursor-pointer">
                          موافقة ✅
                        </button>
                        <button onclick="rejectDeposit('<?= $txId ?>')" class="px-3 py-1.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold text-xs shadow-md transition cursor-pointer">
                          رفض ❌
                        </button>
                      </div>
                    <?php else: ?>
                      <span class="text-slate-500 text-[11px]">تمت المعالجة</span>
                    <?php endif; ?>
                  </td>
                </tr>
                <?php endforeach; ?>
          ...
          <table class="w-full text-right text-xs">
            ...
            <tbody class="divide-y divide-slate-800/60 font-medium">
              <?php if (empty($withdrawsList)): ?>
                <tr>
                  <td colspan="6" class="p-8 text-center text-slate-500">لا توجد طلبات سحب حالياً</td>
                </tr>
              <?php else: ?>
                ...
                <tr class="hover:bg-slate-800/40 transition">
                  ...
                  <td class="p-3.5 text-center">
                    <?php if ($wStatus === 'pending'): ?>
                      <div class="flex items-center justify-center gap-1.5">
                        <button onclick="approveWithdraw('<?= $wId ?>')" class="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-md transition cursor-pointer">
                          تم التحويل ✅
                        </button>
                        <button onclick="rejectWithdraw('<?= $wId ?>')" class="px-3 py-1.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold text-xs shadow-md transition cursor-pointer">
                          رفض واسترجاع ❌
                        </button>
                      </div>
                    <?php else: ?>
